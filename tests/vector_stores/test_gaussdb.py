...
def test_list_combined_numeric_and_datetime_ranges_infer_independently():
    ...
    assert params[5].startswith("^[[:digit:]]{4}-[[:digit:]]{2}-[[:digit:]]{2}T")
    assert params[6] == "created_at"
...
def test_init_auto_create_verifies_bm25_present_when_collection_exists():
    """When collection exists and BM25 index is present, bm25_enabled stays True."""
    mock_conn = MagicMock()
    mock_conn.get_parameter_status.return_value = "UTF8"
    mock_cursor = MagicMock()
    mock_conn.cursor.return_value = mock_cursor
    # fetchone returns a row -> BM25 index exists
    ...
    with patch.object(GaussDB, "_create_connection_pool", return_value=mock_pool), patch.object(
        GaussDB, "list_cols", return_value=["test_collection"]
...
def test_init_auto_create_detects_bm25_absent_when_collection_exists():
    """When collection exists but BM25 index is absent, bm25_enabled becomes False."""
    mock_conn = MagicMock()
    mock_conn.get_parameter_status.return_value = "UTF8"
    mock_cursor = MagicMock()
    mock_conn.cursor.return_value = mock_cursor
    # fetchone returns None -> no BM25 index found
...
class TestLogicBugAndEdgeCases:
    ...
    def test_update_empty_payload_nulls_all_scope_columns(self):
        ...
        assert '"agent_id" = %s' in sql
        assert '"run_id" = %s' in sql
        # payload_value of {} is '{}', memory is None, text_lemmatized is None
        assert params[0] == "{}"
        assert params[1] is None  # memory
        assert params[2] is None  # text_lemmatized
        assert params[3] is None  # user_id
        assert params[4] is None  # agent_id
        assert params[5] is None  # run_id

    # --- H: update() with payload containing only scope keys ---
    def test_update_payload_with_scope_keys_preserves_them_but_nulls_memory(self):
        """When payload has scope keys but no 'data' or 'memory' key,
        memory and text_lemmatized become None while scope columns are preserved."""
        db, _, _, mock_cursor = make_gaussdb()

        db.update("id1", payload={"user_id": "u2", "agent_id": "a1"})

        params = mock_cursor.execute.call_args.args[1]
        # memory and text_lemmatized are None because no 'data' or 'memory' key
        assert params[1] is None  # memory
        assert params[2] is None  # text_lemmatized
        assert params[3] == "u2"  # user_id preserved
        assert params[4] == "a1"  # agent_id preserved
        assert params[5] is None  # run_id not in payload -> NULL

    # --- H: _decode_payload with bytes input reveals a real bug ---
    def test_decode_payload_bytes_input_raises_typeerror_documenting_bug(self):
        """psycopg2 may return JSONB columns as bytes. _decode_payload does NOT
        handle bytes -- it falls through to dict(payload) which raises TypeError.
        This test documents the bug: bytes input is not decoded to dict.
        A fix would add: if isinstance(payload, bytes): return json.loads(payload.decode('utf-8'))"""
        # bytes containing valid JSON falls through the isinstance checks
        # (not None, not dict, not str) and hits dict(payload) -> TypeError
        with pytest.raises(TypeError):
            GaussDB._decode_payload(b'{"key": "value"}')

        # bytes that is a dict-like iterable also fails
        with pytest.raises(TypeError):
            GaussDB._decode_payload(b'{"k":1}')

        # This confirms bytes is a gap in _decode_payload.
        # psycopg2 JSONB columns typically return dict or str, not bytes,
        # but if the connection uses byte encoding, this would be a crash.
    ...
    def test_search_l2_metric_does_not_clamp_negative_distance(self):
        """L2 distance should also be clamped to >= 0 like cosine, since
        numeric errors in GaussDB could produce tiny negatives for L2 too."""
        db, _, _, mock_cursor = make_gaussdb(vector_metric="l2")
        # Simulate a tiny negative L2 distance from floating-point error
        mock_cursor.fetchall.return_value = [("id1", -1e-8, {"data": "hello"})]

        results = db.search("hello", [0.1, 0.2, 0.3], top_k=5)

        # The code clamps for both cosine and l2: max(0.0, distance)
        assert results[0].score == 0.0

    # --- M: search() with L2 metric, large positive distance is NOT clamped ---
    def test_search_l2_metric_keeps_large_positive_distance(self):
        """L2 distances can be arbitrarily large; they should NOT be clamped
        to a fixed upper bound (only negative values are clamped to 0)."""
        db, _, _, mock_cursor = make_gaussdb(vector_metric="l2")
        mock_cursor.fetchall.return_value = [("id1", 999.99, {"data": "hello"})]

        results = db.search("hello", [0.1, 0.2, 0.3], top_k=5)

        assert results[0].score == pytest.approx(999.99)
    ...
    def test_keyword_search_no_filters_produces_where_prefix_only(self):
        ...
        # No scope column filter conditions (payload->>%s or payload @> are filter patterns)
    ...
    # --- M: insert() MERGE MATCHED branch updates updated_at ---
    def test_insert_merge_matched_branch_includes_updated_at(self):
        """The MERGE INTO WHEN MATCHED clause must include updated_at = CURRENT_TIMESTAMP
        to properly update existing rows' timestamps on upsert."""
        db, _, _, mock_cursor = make_gaussdb()
        ...
        sql = executed_sql(mock_cursor)
        # WHEN MATCHED THEN UPDATE SET must include updated_at
        matched_section = sql.split("WHEN MATCHED THEN")[1].split("WHEN NOT MATCHED")[0]
        assert "updated_at = CURRENT_TIMESTAMP" in matched_section
    ...
    def test_insert_row_with_none_payload_defaults_to_empty_dict(self):
        ...
        producing None for memory, text_lemmatized, and all scope columns."""
        db, *_ = make_gaussdb()
        row = db._insert_row([1.0, 2.0, 3.0], None, "test-id")

        assert row[0] == "test-id"
        assert row[2] == "{}"  # _payload_value({}) -> '{}'
        assert row[3] is None  # memory from {} -> None
        assert row[4] is None  # text_lemmatized from {} -> None
        assert row[5] is None  # user_id from {} -> None
        assert row[6] is None  # agent_id from {} -> None
        assert row[7] is None  # run_id from {} -> None

    # --- M: _insert_row with payload containing 'memory' key (not 'data') ---
    def test_insert_row_with_memory_key_fills_memory_field(self):
        """When payload has 'memory' but not 'data', memory field should come from 'memory'."""
        db, *_ = make_gaussdb()
        row = db._insert_row([1.0, 2.0, 3.0], {"memory": "my memory"}, "test-id")

        assert row[3] == "my memory"
        # text_lemmatized falls back to memory when no explicit text_lemmatized key
        assert row[4] == "my memory"

    # --- M: _insert_row with explicit text_lemmatized overrides memory fallback ---
    def test_insert_row_with_explicit_text_lemmatized_overrides_memory_fallback(self):
        ...
        assert row[3] == "raw data"  # memory = data
        assert row[4] == "lemmatized"  # text_lemmatized = explicit key, not fallback

    # --- M: search() cosine large distance (above 1.0) passes through ---
    def test_search_cosine_large_distance_above_one_not_clamped(self):
        """Cosine distance from <+> can exceed 1.0 in GaussDB; only negatives
        are clamped, not values above the theoretical cosine range."""
        db, _, _, mock_cursor = make_gaussdb()
        mock_cursor.fetchall.return_value = [("id1", 1.5, {"data": "hello"})]
